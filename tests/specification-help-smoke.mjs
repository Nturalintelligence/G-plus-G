...
try {
  ...
  const assertCaptionGeometry = async (label) => {
    const geometry = await page.evaluate(() => {
      ...
      const titlebarBottom = overlay ? overlay.y + overlay.height : header?.bottom ?? 56;
      ...
      return { close: close && { left: close.left, top: close.top, right: close.right, bottom: close.bottom }, backdropTop: backdrop?.top, titlebarBottom, caption: { left: captionLeft, top: 0, right: innerWidth, bottom: titlebarBottom }, hitIsClose: hit === closeElement || !!hit?.closest(".spec-help-header button"), appRegion: style?.getPropertyValue("-webkit-app-region") };
    });
