...
| Attachment composer and preview polish | Composer attachment strip / preview modal | renderer thumbnail/document cards and bounded integrity preview | TESTED | UI contract + packaged visual matrix | PNG/JPEG/WebP, wide/tall images, multiple cards, removal, light/dark and 100/125/150% checked. |
| Model discussion presentation | Ready answer action | persisted RIGHT_DRAWER/FULLSCREEN setting, independent scroll, narrow-window fullscreen | TESTED | settings/component + packaged visual matrix | Stopping policy remains a separate known issue. |
...
| Provider-specific attachment delivery | Send with attachments | capability policy, integrity recheck, exact FileList, provider composer evidence, Delivery and Submission FSM | IMPLEMENTED | real local Chromium fixtures using production ChatGPT/Gemini selectors; PNG/JPEG/WebP/PDF/MD matrix; FSM/no-blind-retry integration | Live ChatGPT and Gemini must confirm the same matrix after explicit owner approval. Local fixtures are not live evidence; UNKNOWN is never retried blindly. |
