...
function registerIpc(): void {
  ...
  handle("projects:delete", async (_event, input: unknown) => {
    ...
    const repository = new ProjectRepository(db());
    const activeCliTask = db().raw
      .prepare(
        `SELECT task_id, status FROM cli_tasks
         WHERE project_id = ? AND status IN ('QUEUED', 'RUNNING', 'VERIFYING')
         LIMIT 1`,
      )
      .get(projectId) as { task_id: string; status: string } | undefined;
    if (activeCliTask) {
      throw new Error(
        `Нельзя удалить проект: CLI-задача ${activeCliTask.task_id} находится в состоянии ${activeCliTask.status}`,
      );
    }
    const remoteResults: Array<{ providerId: string; conversationId: string; deleted: boolean; error?: string }> = [];
    const ownedAdapters = new Map<string, ModelAdapter>();
    if (deleteRemote) {
      if (activeOrchestrator || activeOrchestrationAdapters || activeInteractiveLogin) {
        throw new Error("Нельзя удалять веб-чаты во время активной операции");
      }
      ...
      try {
        for (const conv of conversations) {
          ...
          try {
            let adapter = ownedAdapters.get(conv.providerId);
            if (!adapter) {
              adapter = createAdapter(parseProvider(conv.providerId), 60_000, true);
              await adapter.launch();
              const session = await adapter.checkSession();
              if (session !== "AUTHENTICATED") {
                throw new Error(`${conv.providerId}: требуется авторизация (${session})`);
              }
              ownedAdapters.set(conv.providerId, adapter);
            }
            if (typeof adapter.deleteConversation !== "function") {
              throw new Error(`${conv.providerId}: удаление веб-чата не поддерживается`);
            }
            const deleted = await adapter.deleteConversation({ id: conv.id, url: conv.externalRef });
            if (!deleted) throw new Error(`${conv.providerId}: подтверждение удаления не найдено`);
            remoteResults.push({ providerId: conv.providerId, conversationId: conv.id, deleted: true });
          } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            remoteResults.push({ providerId: conv.providerId, conversationId: conv.id, deleted: false, error: message });
        ...
        await Promise.allSettled([...ownedAdapters.values()].map((adapter) => adapter.close()));
      }
      const failed = remoteResults.filter((result) => !result.deleted);
      if (failed.length > 0) {
        return { success: false, localDeleted: false, remoteResults };
      }
    }

    const artifactStore = new LocalArtifactStore();
    const artifactProjectDirectory = artifactStore.resolveAbsolutePath(projectId);
    repository.deleteProject(projectId);
    let artifactCleanupError: string | undefined;
    try {
      if (fs.existsSync(artifactProjectDirectory)) {
        fs.rmSync(artifactProjectDirectory, { recursive: true, force: false });
      }
    } catch (error) {
      artifactCleanupError = error instanceof Error ? error.message : String(error);
      logEvent("WARN", "project.delete.artifact_cleanup_failed", {
        projectId,
        error: artifactCleanupError,
      });
    }
    ...
    return {
      ...
      localDeleted: true,
      remoteResults,
      ...(artifactCleanupError ? { artifactCleanupError } : {}),
    };
  });
  ...
  handle("provider:status", async (_event, providerValue: unknown) => {
    ...
    if (process.env.G_PLUS_G_SKIP_PROVIDER_STATUS === "1") {
      return { provider, session: "UNKNOWN", ready: false, skipped: true };
    }
